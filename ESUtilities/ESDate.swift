...
public class ESDate:NSObject {
    ...
    public class func getHumanDate(fromDate date:Date , localized:Bool = false)->String{
        ...
        if currentDate.years(from: date)   > 0 {
            return localized ? "\(currentDate.years(from: date))" + ESUtilities.localizedSitringFor(key: "ESYear") : "\(currentDate.years(from: date))y"
        }
        if currentDate.months(from: date)  > 0 {
            return localized ? "\(currentDate.months(from: date))" + ESUtilities.localizedSitringFor(key: "ESMonth") : "\(currentDate.months(from: date))M"
        }
        if currentDate.weeks(from: date)   > 0 {
            return localized ? "\(currentDate.weeks(from: date))" + ESUtilities.localizedSitringFor(key: "ESWeek") : "\(currentDate.weeks(from: date))w"
        }
        if currentDate.days(from: date)    > 0 {
            return localized ? "\(currentDate.days(from: date))" + ESUtilities.localizedSitringFor(key: "ESDay") : "\(currentDate.days(from: date))d"
        }
        if currentDate.hours(from: date)   > 0 {
            return localized ? "\(currentDate.hours(from: date))" + ESUtilities.localizedSitringFor(key: "ESHour") : "\(currentDate.hours(from: date))h"
        }
        if currentDate.minutes(from: date) > 0 {
            return localized ? "\(currentDate.minutes(from: date))" + ESUtilities.localizedSitringFor(key: "ESMinute") : "\(currentDate.minutes(from: date))m"
        }
        if currentDate.seconds(from: date) > 0 {
            return localized ? "\(currentDate.seconds(from: date))" + ESUtilities.localizedSitringFor(key: "ESSecond") : "\(currentDate.seconds(from: date))s"
        }
        return ""
    }
}
